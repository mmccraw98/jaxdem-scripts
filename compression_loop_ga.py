...
def step(state, system, output_dir, config, save_strides = None, compress = True, save_fn = None, save_all = False):
    ...
    if save_all:
        ...
        data = dict(
            ...
            pos_c=traj_state.pos_c[:, idx],
            q_w=traj_state.q.w[:, idx],
            q_xyz=traj_state.q.xyz[:, idx],
            vel=traj_state.vel[:, idx],
            ang_vel=traj_state.ang_vel[:, idx],
